...
# import train_skf
# train_skf.seed_everything(42) # randomseed42
...
class MaskSplitByProfileDataset(MaskBaseDataset):
    ...
    #원래는 mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246)
...
#         }
    ...
    def _split_profile(profiles, val_ratio):
        ...
        df = pd.DataFrame(None, columns = ['path', 'age', 'idx'])
        for idx, phase in enumerate(profiles):
            age = age_cls(int(phase.split('_')[3]))
            path = phase.split('_')[0]
...
def getDataloader(dataset, train_idx, valid_idx, batch_size, valid_batch_size, num_workers):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        # sampler=ImbalancedDatasetSampler(train_set, callback_get_label=get_labels),
        drop_last=True,
        shuffle=True,
