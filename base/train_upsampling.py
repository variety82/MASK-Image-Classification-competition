...
def train(data_dir, model_dir, args):
    ...
    # print(max(dataset.encode_multi_class(dataset.mask_labels,dataset.gender_labels,dataset.age_labels)))# -> 라벨값
    # print((dataset.indices["val"]))
    # for i in range(20):
    #     print(next(iter))
    # print(list(dataset.get_labels()))
    # get_label = dataset.get_labels()
    # print(len(get_label))
    # print(len(train_set))
    # print(get_label.reshape(-1,1))
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        # sampler=ImbalancedDatasetSampler(dataset, indices=dataset.indices["train"], callback_get_label=get_label),
        pin_memory=use_cuda,
        drop_last=False,
    )
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                # pred_items.extend(np.array(pred_item.cpu()))
            ...
            f1 = np.sum(f1_items) / len(val_loader)
            # f1 = f1_score(labels.cpu(), pred_items ,average="macro") / len(val_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--valid_batch_size', type=int, default=3780, help='input batch size for validing (default: 1000)')
