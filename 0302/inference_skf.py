...
def inference(data_dir, model_dir, output_dir, args):
    ...
    for i in range(5):
        ...
        with torch.no_grad():
            ...
            pred_id = pd.DataFrame(dict(ImageID= info['ImageID']))
    ...
    submission_arithmetic = pd.DataFrame(dict(ImageID=info['ImageID'], ans=ans_arithmetic))
    submission_arithmetic.to_csv('submission_ens_arithmetic.csv', index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='Backbone', help='model type (default: BaseModel)')

    # Container environment
    parser.add_argument('--data_dir', type=str, default=os.environ.get('SM_CHANNEL_EVAL', '/opt/ml/input/data/eval'))
    parser.add_argument('--model_dir', type=str, default=os.environ.get('SM_CHANNEL_MODEL', './model/skf59_test'))
    parser.add_argument('--output_dir', type=str, default=os.environ.get('SM_OUTPUT_DATA_DIR', './output/skf59_test'))
