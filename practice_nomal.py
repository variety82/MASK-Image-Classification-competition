...
class AlbumAugmentation:
    def __init__(self, resize, mean, std, **args):
        ...
        self.transform = A.Compose([
            A.CenterCrop(always_apply=False, p=1.0, height=320, width=256),
            A.Resize(h, w, interpolation = 0),
            A.ShiftScaleRotate(always_apply=False, p=0.2, 
                shift_limit=(-0.05999999865889549, 0.05999999865889549), 
                scale_limit=(-0.10000000149011612, 0.10000000149011612), 
                rotate_limit=(-90, 90), interpolation=0, border_mode=0, value=(0, 0, 0), mask_value=None),
...
class TestDataset(Dataset):
    def __init__(self, img_paths, resize, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246)):
        ...
        self.transform = Compose([
            ...
            Normalize(mean=mean, std=std, max_pixel_value = 255.0, p= 1),
            ToTensorV2(p=1.0)
        ], p =1.0)
...
# print(args)
dataset_module = MaskBaseDataset  # default: BaseAugmentation
...
#             ]
# )
transform = get_transforms(img_size = (128,96))
dataset.set_transform(transform['train'])


        # -- data_loader
train_set, val_set = dataset.split_dataset()
print(train_set.dataset.mean, val_set.dataset.mean)
